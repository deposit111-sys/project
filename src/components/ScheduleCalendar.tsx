      ...
      >
        {isFullscreen ? (
          ...
          >
            <thead>
              <tr>
                <th className="border border-gray-300 py-2 px-3 bg-gray-50 sticky left-0 top-0 z-50 font-semibold">日期/编号</th>
                {daysInMonth.map(date => (
                  <th 
                    key={date.toISOString()} 
                    className="border border-gray-300 py-2 px-3 bg-gray-50 min-w-[80px] font-semibold cursor-pointer hover:bg-gray-100 transition-colors duration-200 sticky top-0 z-40"
                    onMouseEnter={(e) => handleMouseEnter(date, e)}
                    title="悬停查看当日订单详情"
                  >
                    {date.getDate()}日
            ...
            <tbody>
              {sortedCameras.map(camera => (
                <tr key={camera.id}>
                  <td className="border border-gray-300 p-3 bg-gray-50 sticky left-0 z-30">
                    <div className="text-sm">
                      <div className="font-medium">{camera.model}</div>
                      <div className="text-gray-600">{camera.serialNumber}</div>
                    </div>
