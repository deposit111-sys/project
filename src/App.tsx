...
function App() {
  ...
  const [useDatabase, setUseDatabase] = useLocalStorage<boolean>('useDatabase', false);
  ...
  // 根据数据库连接状态决定使用哪套数据
  const currentCameras = useDatabase && databaseConnected ? dbCameras : cameras;
  const currentOrders = useDatabase && databaseConnected ? dbOrders : orders;
  const currentConfirmedPickups = useDatabase && databaseConnected ? dbConfirmedPickups : confirmedPickups;
  const currentConfirmedReturns = useDatabase && databaseConnected ? dbConfirmedReturns : confirmedReturns;
